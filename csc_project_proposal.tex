...
%Grzesiek's comment:
%In this fragment you say that an attention budget is agent dependent. I thought we were talking about an budget? If we do not stipulate something like that our model indeed becomes more socially plausible, but on the other hand it would make it harder for us to model as we would probably need to define the budget building up on individual budget's.
%Silvan's reply: I thought that we could start out by a fixed budget that is the same for all readers. At a later stage, however, we might want to change this parameter. That's why I left it unclear at this point.
%Still: I now notice that my formal definition doesn't make any sense then. I will change it to "for all players". If we later want to change it, we can still assume a budget vector or something.
...
%Silvan's reply: I added some detail and I think Max's part, which largely overlaps with this one right here, gives more detail as well.
